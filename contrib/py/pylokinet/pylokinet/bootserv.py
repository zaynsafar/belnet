#!/usr/bin/env python3
#
# python wsgi application for managing many belnet instances
#

__doc__ = """belnet bootserv wsgi app
also handles webhooks for CI
run me with via gunicorn pybelnet.bootserv:app
"""
...
def handle_serve_belnet(modified_since, respond):
    l = BinHolder('belnet.zip')
    return l.serve(modified_since, respond)


def fetch_belnet(j, ref="staging", name="build:linux"):
    holder = BinHolder("belnet.zip")
    if 'builds' not in j:
        return False
    selected = None
    attrs = dict()
...
def app(environ, start_response):
    ...
    if method.upper() == "PUT" and request_body_size > 0:
        ...
        elif environ.get("PATH_INFO") == "/belnet.zip":
            return handle_serve_belnet(environ.get("HTTP_IF_MODIFIED_SINCE"),start_response)
        elif environ.get("PATH_INFO") == "/":
            return response("200 OK", "belnet bootserv", start_response)
        else:
