...
class Context:
    """
    wrapper around libbelnet
    """

    def __init__(self, debug=False):
        self._ln = ctypes.CDLL(find_library("belnet"))
        self._c = ctypes.CDLL(find_library("c"))
        self._ln.belnet_context_new.restype = ctypes.POINTER(LNContext)
        self._ln.belnet_address.restype = ctypes.c_char_p
        self._ln.belnet_address.argtypes = (ctypes.POINTER(LNContext), )
        self._ln.belnet_outbound_stream.restype = ctypes.POINTER(ResultStruct)
        self._ln.belnet_outbound_stream.argtypes = (ctypes.POINTER(ResultStruct), ctypes.c_char_p, ctypes.c_char_p, ctypes.POINTER(LNContext))
        self._ctx = self._ln.belnet_context_new()
        self._addrmap = dict()
        self._debug = debug
        lvl = 'none'
        if self._debug:
            lvl = 'debug'
        self._ln.belnet_log_level(ctypes.create_string_buffer(lvl.encode('ascii')))
    ...
    def add_bootstrap(self, data):
        ...
        return self.ln_call("belnet_add_bootstrap_rc", ptr, ptrlen)

    def wait_for_ready(self, ms):
        return self.ln_call("belnet_wait_for_ready", ms) == 0

    def ready(self):
        return self.ln_call("belnet_status") == 0

    def addr(self):
        return self._ln.belnet_address(self._ctx).decode('ascii')

    def expose(self, port):
        return self.ln_call('belnet_inbound_stream', port)
    ...
    def start(self):
        return self.ln_call("belnet_context_start")

    def stop(self):
        self.ln_call("belnet_context_stop")
    ...
    def __del__(self):
        self.stop()
        self._ln_call("belnet_context_free")

    def set_netid(self, netid):
        self._ln.belnet_set_netid(ctypes.create_string_buffer(netid.encode('ascii')))
...
class Stream:
    ...
    def connect(self, remote):
        result = ResultStruct()
        self._ctx.ln_call("belnet_outbound_stream", ctypes.cast(ctypes.addressof(result), ctypes.POINTER(ResultStruct)), ctypes.create_string_buffer(remote.encode()), ctypes.c_char_p(0))
    ...
    def close(self):
        if self._id is not None:
            self._ctx.ln_call("belnet_close_stream", self._id)
...
try:
    while not ctx.wait_for_ready(500):
        print("waiting for belnet...")
    beldexaddr = ctx.addr()
    print("we are {}".format(beldexaddr))
    if args.expose:
        id = ctx.expose(args.expose)
        print("exposed {}:{}".format(beldexaddr, args.expose))
    print("serving on {}:{}".format(*addr))
    server.serve_forever()
finally:
    if id is not None:
        ctx.ln_call("belnet_close_stream", id)
    ctx.stop()
