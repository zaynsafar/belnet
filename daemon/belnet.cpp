...
startWinsock()
{
  ...
  ::CreateMutex(nullptr, FALSE, "belnet_win32_daemon");
  return 0;
}
...
run_main_context(std::optional<fs::path> confFile, const llarp::RuntimeOptions opts)
{
  ...
  try
  {
    ...
    if (not conf->Load(confFile, opts.isMNode))
    {
...
belnet_main(int argc, char* argv[])
{
  ...
  if (configFile.has_value())
  {
    ...
    if (genconfigOnly)
    {
      try
      {
        llarp::ensureConfig(basedir, *configFile, overwrite, opts.isMNode);
      }
      catch (std::exception& ex)
      {
  ...
  else
  {
    ...
    {
      llarp::ensureConfig(
          llarp::GetDefaultDataDir(), llarp::GetDefaultConfigPath(), overwrite, opts.isMNode);
    }
    catch (std::exception& ex)
    {
