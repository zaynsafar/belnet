  ...
  class TunnelManager
  {
   public:
    using ListenHandler = std::function<std::optional<SockAddr>(
        std::string_view belnet_addr,  // The remote's full belnet address
    ...
    ///
    /// This call immediately opens the local TCP socket, and initiates the belnet connection and
   ...
   private:
    ...
    allow_connection(std::string_view belnet_addr, uint16_t port);
