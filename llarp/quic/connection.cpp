...
#pragma GCC diagnostic pop
  ...
  // We add some belnet-specific data into the transport request and *always* as the first
  ...
  static constexpr uint64_t belnet_transport_param_N = 42;
  static constexpr auto belnet_metadata_code_raw =
      encode_varint(31 * belnet_transport_param_N + 27);
  static constexpr std::basic_string_view<uint8_t> belnet_metadata_code{
      belnet_metadata_code_raw.first.data(), belnet_metadata_code_raw.second};
  static_assert(
      belnet_metadata_code.size() == 2 && belnet_metadata_code[0] == 0b01000101
      && belnet_metadata_code[1] == 0b00110001);

  int
  Connection::recv_transport_params(std::basic_string_view<uint8_t> data)
  {
    if (data.substr(0, belnet_metadata_code.size()) != belnet_metadata_code)
    {
      LogWarn("transport params did not begin with expected belnet metadata");
      return NGTCP2_ERR_TRANSPORT_PARAM;
    }
    auto [meta_len, meta_len_bytes] = decode_varint(data.substr(belnet_metadata_code.size()));
    if (meta_len_bytes == 0)
    {
      LogWarn("transport params belnet metadata has truncated size");
      return NGTCP2_ERR_MALFORMED_TRANSPORT_PARAM;
    }
    std::string_view belnet_metadata{
        reinterpret_cast<const char*>(
            data.substr(belnet_metadata_code.size() + meta_len_bytes).data()),
        static_cast<std::size_t>(meta_len)};
    LogDebug("Received bencoded belnet metadata: ", buffer_printer{belnet_metadata});

    uint16_t port;
    try
    {
      oxenmq::bt_dict_consumer meta{belnet_metadata};
      ...
      LogDebug("decoded belnet tunnel port = ", port);
    }
    catch (const oxenmq::bt_deserialize_invalid& c)
    {
      LogWarn("transport params belnet metadata is invalid: ", c.what());
      return NGTCP2_ERR_TRANSPORT_PARAM;
    }
  ...
  Connection::send_transport_params(ngtcp2_crypto_level level)
  {
    ...
    {
      // Send our first parameter, the belnet metadata, in a QUIC-compatible way (by using a
      ...
      std::string belnet_metadata = bt_serialize(oxenmq::bt_dict{
          {"#", tunnel_port},
      });
      copy_and_advance(buf, belnet_metadata_code);
      auto [bytes, size] = encode_varint(belnet_metadata.size());
      copy_and_advance(buf, std::basic_string_view{bytes.data(), size});
      copy_and_advance(buf, belnet_metadata);
      assert(buf < bufend);
    }
