        ...
        .add_request_command(
            ...
            [&](oxenmq::Message& msg) {
              HandleJSONRequest(msg, [r = m_Router](nlohmann::json obj, ReplyFunction_t reply) {
                ...
                RouterID routerID;
                if (auto itr = obj.find("mnode"); itr != obj.end())
                {
                ...
                r->loop()->call([r, endpoint, routerID, reply]() {
                  ...
                  ep->ObtainMNodeSession(routerID, [routerID, ep, reply](auto session) {
                    if (session and session->IsReady())
                    {
                      ...
                      reply(CreateJSONError("failed to obtain mnode session"));
                  });
                });
              });
            })
