...
namespace llarp
{
  namespace service
  {
    ...
    Endpoint::GetEndpointWithConvoTag(ConvoTag tag) const
    {
      ...
      for (const auto& item : m_state->m_MNodeSessions)
      {
    ...
    void Endpoint::Tick(llarp_time_t)
    {
      ...
      // expire mnode sessions
      EndpointUtil::ExpireMNodeSessions(now, m_state->m_MNodeSessions);
      // expire pending tx
    ...
    Endpoint::ResetInternalState()
    {
      ...
      resetState(m_state->m_MNodeSessions, [](const auto& item) { return item.second; });
    }
    ...
    {
      return path::Builder::GetHopsAlignedToForBuild(endpoint, MnodeBlacklist());
    }
    ...
    {
      return {{"BELNET_ADDR", m_Identity.pub.Addr().ToString()}};
    }
    ...
    bool
    Endpoint::HasPathToMNode(const RouterID ident) const
    {
      auto range = m_state->m_MNodeSessions.equal_range(ident);
    ...
    bool
    Endpoint::EnsurePathToMNode(const RouterID mnode, MNodeEnsureHook h)
    {
      auto& nodeSessions = m_state->m_MNodeSessions;

      using namespace std::placeholders;
      if (nodeSessions.count(mnode) == 0)
      {
        const auto src = xhtonl(net::TruncateV6(GetIfAddr()));
        const auto dst = xhtonl(net::TruncateV6(ObtainIPForAddr(mnode)));
        ...
        auto session = std::make_shared<exit::MNodeSession>(
            ...
            [=](const llarp_buffer_t& buf) -> bool {
              ...
              /// TODO: V6
              auto itr = m_state->m_MNodeSessions.find(mnode);
              if (itr == m_state->m_MNodeSessions.end())
                return false;
            ...
            false,
            this);
        m_state->m_MNodeSessions[mnode] = session;
      }
      EnsureRouterIsKnown(mnode);
      auto range = nodeSessions.equal_range(mnode);
      auto itr = range.first;
      while (itr != range.second)
      {
        if (itr->second->IsReady())
          h(mnode, itr->second, ConvoTag{itr->second->CurrentPath()->as_array()});
        else
        {
          itr->second->AddReadyHook([h, mnode](auto session) {
            if (session)
            {
              h(mnode, session, ConvoTag{session->CurrentPath()->as_array()});
            }
            else
            {
              h(mnode, nullptr, ConvoTag{});
            }
          });
    ...
    {
      LogTrace("SendToOrQueue: sending to mnode ", addr);
    ...
    {
      FlushRecvData();
      // send downstream packets to user for mnode
      for (const auto& [router, session] : m_state->m_MNodeSessions)
    ...
    Endpoint::GetBestConvoTagFor(std::variant<Address, RouterID> remote) const
    {
      ...
      {
        auto itr = m_state->m_MNodeSessions.find(*ptr);
        if (itr == m_state->m_MNodeSessions.end())
          return std::nullopt;
        ...
        llarp_time_t timeout)
    {
      ...
      {
        return EnsurePathToMNode(*ptr, [hook](auto, auto session, auto tag) {
          if (session)
          {
            hook(tag);
          }
    ...
    void
    Endpoint::BlacklistMNode(const RouterID mnode)
    {
      m_state->m_MnodeBlacklist.insert(mnode);
    }

    const std::set<RouterID>&
    Endpoint::MnodeBlacklist() const
    {
      return m_state->m_MnodeBlacklist;
    }
