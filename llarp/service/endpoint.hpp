...
namespace llarp
{
  ...
  namespace service
  {
                      ...
                      public EndpointBase
    {
      ...
      /// handle packet io from master node or hidden service to frontend
      virtual bool
      HandleInboundPacket(
      ...
      void
      BlacklistMNode(const RouterID mnode) override;
      ...
      using MNodeEnsureHook = std::function<void(const RouterID, exit::BaseSession_ptr, ConvoTag)>;

      void
      InformPathToService(const Address remote, OutboundContext* ctx);

      /// ensure a path to a master node by public key
      bool
      EnsurePathToMNode(const RouterID remote, MNodeEnsureHook h);
      ...
      bool
      HasPathToMNode(const RouterID remote) const;
