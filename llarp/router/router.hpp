...
#include <llarp/rpc/beldexd_rpc_client.hpp>
...
namespace llarp
{
  struct Router : public AbstractRouter
  {
    ...
    /// should we obey the master node whitelist?
    ...
    const std::shared_ptr<rpc::BeldexdRpcClient>&
    RpcClient() const override
    {
      return m_beldexdRpcClient;
    }
    ...
    /// return true if we look like we are a decommissioned master node
    bool
    ...
    /// initialize us as a master node
    ...
    bool
    Configure(std::shared_ptr<Config> conf, bool isMNode, std::shared_ptr<NodeDB> nodedb) override;
    ...
    /// count the number of unique master nodes connected via pubkey
    size_t
