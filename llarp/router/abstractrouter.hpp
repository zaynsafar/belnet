...
namespace llarp
{
  ...
  struct AbstractRouter : public std::enable_shared_from_this<AbstractRouter>
  {
    ...
    virtual const std::shared_ptr<rpc::BeldexdRpcClient>&
    RpcClient() const = 0;
    ...
    virtual bool
    Configure(std::shared_ptr<Config> conf, bool isMNode, std::shared_ptr<NodeDB> nodedb) = 0;
    ...
    /// count the number of master nodes we are connected to
    virtual size_t
    ...
    /// if we are a master node this is done direct otherwise it's done via
    /// path
    virtual void
