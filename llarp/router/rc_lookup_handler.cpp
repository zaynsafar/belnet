...
namespace llarp
{
      ...
      const std::vector<RouterID>& whitelist, const std::vector<RouterID>& greylist)
  {
    ...
    LogInfo("belnet master node list now has ", whitelistRouters.size(), " routers");
  }
  ...
  RCLookupHandler::ExploreNetwork()
  {
    ...
    // master nodes gossip, not explore
    if (_dht->impl->GetRouter()->IsMasterNode())
      return;
