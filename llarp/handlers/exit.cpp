...
namespace llarp
{
  namespace handlers
  {
    ...
    ExitEndpoint::GetEndpointWithConvoTag(service::ConvoTag tag) const
    {
      ...
      for (const auto& [rid, session] : m_MNodeSessions)
      {
    ...
    ExitEndpoint::GetBestConvoTagFor(AddressVariant_t addr) const
    {
      if (auto* rid = std::get_if<RouterID>(&addr))
      {
        ...
        if (VisitEndpointsFor(PubKey{*rid}, visit) and not tag.IsZero())
          return tag;
        auto itr = m_MNodeSessions.find(*rid);
        if (itr == m_MNodeSessions.end())
        {
          return std::nullopt;
        }
        ...
        service::ConvoTag tag, const llarp_buffer_t& payload, service::ProtocolType type)
    {
      if (auto maybeAddr = GetEndpointWithConvoTag(tag))
      {
        ...
        if (auto* rid = std::get_if<RouterID>(&*maybeAddr))
        {
          ...
          ObtainMNodeSession(*rid, [data = payload.copy(), type](auto session) {
        ...
        llarp_time_t)
    {
      ...
      if (auto* rid = std::get_if<RouterID>(&addr))
      {
        if (m_MNodeKeys.count(PubKey{*rid}) or m_Router->PathToRouterAllowed(*rid))
        {
          ObtainMNodeSession(
    ...
    ExitEndpoint::HandleHookedDNSMessage(dns::Message msg, std::function<void(dns::Message)> reply)
    {
      if (msg.questions[0].qtype == dns::qTypePTR)
      {
        ...
        else
        {
          ...
          if (itr != m_IPToKey.end() && m_MNodeKeys.find(itr->second) != m_MNodeKeys.end())
          {
      ...
      {
        if (msg.questions[0].IsName("random.mnode"))
        {
          RouterID random;
      ...
      else if (msg.questions[0].qtype == dns::qTypeA || msg.questions[0].qtype == dns::qTypeAAAA)
      {
        ...
        if (msg.questions[0].IsName("random.mnode"))
        {
          ...
          {
            msg.AddCNAMEReply(random.ToString(), 1);
            auto ip = ObtainMasterNodeIP(random);
        ...
        if (r.FromString(msg.questions[0].Name()))
        {
          ...
          else if (m_MNodeKeys.find(pubKey) == m_MNodeKeys.end())
          {
          ...
          {
            // we have it mapped already as a master node
    ...
    void
    ExitEndpoint::ObtainMNodeSession(const RouterID& router, exit::SessionReadyFunc obtainCb)
    {
      ...
      ObtainMasterNodeIP(router);
      m_MNodeSessions[router]->AddReadyHook(obtainCb);
    }
    ...
    ExitEndpoint::Flush()
    {
      m_InetToNetwork.Process([&](Pkt_t& pkt) {
        ...
        // check if this key is a master node
        if (m_MNodeKeys.count(pk))
        {
          // check if it's a master node session we made and queue it via our
          // mnode session that we made otherwise use an inbound session that
          // was made by the other master node
          auto itr = m_MNodeSessions.find(pk);
          if (itr != m_MNodeSessions.end())
          {
      ...
      for (auto& [id, session] : m_MNodeSessions)
      {
    ...
    bool
    ExitEndpoint::QueueMNodePacket(const llarp_buffer_t& buf, huint128_t from)
    {
      net::IPPacket pkt;
      if (!pkt.Load(buf))
        return false;
    ...
    huint128_t
    ExitEndpoint::ObtainMasterNodeIP(const RouterID& other)
    {
      ...
      if (m_MNodeKeys.emplace(pubKey).second)
      {
        auto session = std::make_shared<exit::MNodeSession>(
            other,
            [this, ip](const auto& buf) { return QueueMNodePacket(buf, ip); },
            GetRouter(),
            2,
            1,
            true,
            this);
        // this is a new master node make an outbound session to them
        m_MNodeSessions[other] = session;
      }
      return ip;
    }
    ...
    ExitEndpoint::AllocateNewExit(const PubKey pk, const PathID_t& path, bool wantInternet)
    {
      ...
      {
        // we think this path belongs to a master node
        // mark it as such so we don't make an outbound session to them
        m_MNodeKeys.emplace(pk.as_array());
      }
    ...
    {
      {
        auto itr = m_MNodeSessions.begin();
        while (itr != m_MNodeSessions.end())
        {
          if (itr->second->IsExpired(now))
            itr = m_MNodeSessions.erase(itr);
          else
          {
            itr->second->Tick(now);
            ++itr;
