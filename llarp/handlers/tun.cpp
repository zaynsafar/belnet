...
namespace llarp
{
  namespace handlers
  {
        ...
        if (fs::exists(file))
        {
          ...
          if (not data.empty())
          {
            ...
            for (const auto& [key, value] : parsed)
            {
              ...
              if (const auto* beldex = std::get_if<service::Address>(&addr))
              {
                m_IPToAddr.emplace(ip, beldex->data());
                m_AddrToIP.emplace(beldex->data(), ip);
                m_MNodes[*beldex] = false;
                LogInfo(Name(), " remapped ", ip, " to ", *beldex);
              }
              if (const auto* mnode = std::get_if<RouterID>(&addr))
              {
                m_IPToAddr.emplace(ip, mnode->data());
                m_AddrToIP.emplace(mnode->data(), ip);
                m_MNodes[*mnode] = true;
                LogInfo(Name(), " remapped ", ip, " to ", *mnode);
              }
              if (m_NextIP < ip)
                m_NextIP = ip;
    ...
    static bool
    is_random_mnode(const dns::Message& msg)
    {
      return msg.questions[0].IsName("random.mnode");
    }

    static bool
    is_localhost_beldex(const dns::Message& msg)
    {
    ...
    {
      auto ReplyToMNodeDNSWhenReady = [this, reply](RouterID mnode, auto msg, bool isV6) -> bool {
        return EnsurePathToMNode(
            mnode,
            [this, mnode, msg, reply, isV6](
                const RouterID&, exit::BaseSession_ptr s, [[maybe_unused]] service::ConvoTag tag) {
              SendDNSReply(mnode, s, msg, reply, isV6);
            });
      };
      auto ReplyToBeldexDNSWhenReady = [this, reply, timeout = PathAlignmentTimeout()](
      ...
      auto ReplyToDNSWhenReady = [ReplyToBeldexDNSWhenReady, ReplyToMNodeDNSWhenReady](
                                     auto addr, auto msg, bool isV6) {
        if (auto ptr = std::get_if<RouterID>(&addr))
        {
          ReplyToMNodeDNSWhenReady(*ptr, msg, isV6);
          return;
        }
        if (auto ptr = std::get_if<service::Address>(&addr))
        {
          ReplyToBeldexDNSWhenReady(*ptr, msg, isV6);
          return;
        }
      };

      auto ReplyToBeldexSRVWhenReady = [this, reply, timeout = PathAlignmentTimeout()](
      ...
      if (msg.answers.size() > 0)
      {
        ...
        else if (answer.HasCNameForTLD(".bdx"))
        {
          ...
          auto replyMsg = std::make_shared<dns::Message>(clear_dns_message(msg));
          return ReplyToBeldexDNSWhenReady(addr, replyMsg, false);
        }
      }
      ...
      {
        RouterID mnode;
        if (mnode.FromString(qname))
        {
          m_router->LookupRouter(mnode, [reply, msg = std::move(msg)](const auto& found) mutable {
            if (found.empty())
            {
              msg.AddNXReply();
            }
      ...
      else if (msg.questions[0].qtype == dns::qTypeCNAME)
      {
        ...
        else if (is_localhost_beldex(msg))
        {
          size_t counter = 0;
      ...
      else if (msg.questions[0].qtype == dns::qTypeA || msg.questions[0].qtype == dns::qTypeAAAA)
      {
        ...
        else if (is_random_mnode(msg))
        {
          ...
          {
            msg.AddCNAMEReply(random.ToString(), 1);
            return ReplyToMNodeDNSWhenReady(random, std::make_shared<dns::Message>(msg), isV6);
          }
          else
            msg.AddNXReply();
        }
        ...
        else if (addr.FromString(qname, ".bdx"))
        {
          ...
          {
            return ReplyToBeldexDNSWhenReady(addr, std::make_shared<dns::Message>(msg), isV6);
          }
        }
        else if (addr.FromString(qname, ".mnode"))
        {
          if (isV4 && SupportsV6())
          {
            msg.hdr_fields |= dns::flags_QR | dns::flags_AA | dns::flags_RA;
          ...
          {
            return ReplyToMNodeDNSWhenReady(
      ...
      else if (msg.questions[0].qtype == dns::qTypeSRV)
      {
        ...
        else if (addr.FromString(qname, ".bdx"))
        {
          ...
          return ReplyToBeldexSRVWhenReady(addr, std::make_shared<dns::Message>(msg));
        }
      }
      else
      {
    ...
    bool
    TunEndpoint::MapAddress(const service::Address& addr, huint128_t ip, bool MNode)
    {
      auto itr = m_IPToAddr.find(ip);
      if (itr != m_IPToAddr.end())
      {
