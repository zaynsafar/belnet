...
namespace llarp
{
  ...
  namespace handlers
  {
    struct ExitEndpoint : public dns::IQueryHandler, public EndpointBase
    {
      ...
      /// async obtain mnode session and call callback when it's ready to send
      void
      ObtainMNodeSession(const RouterID& router, exit::SessionReadyFunc obtainCb);

     private:
      huint128_t
      AllocateNewAddress();

      /// obtain ip for master node session, creates a new session if one does
      /// not existing already
      huint128_t
      ObtainMasterNodeIP(const RouterID& router);

      bool
      QueueMNodePacket(const llarp_buffer_t& buf, huint128_t from);
      ...
      using MNodes_t = std::set<PubKey>;
      /// set of pubkeys we treat as mnodes
      MNodes_t m_MNodeKeys;

      using MNodeSessions_t = std::unordered_map<RouterID, std::shared_ptr<exit::MNodeSession>>;
      /// mnode sessions we are talking to directly
      MNodeSessions_t m_MNodeSessions;
