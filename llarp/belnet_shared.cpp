...
struct belnet_context
{
  ...
  belnet_context() : impl{std::make_shared<Context>()}, config{llarp::Config::EmbeddedConfig()}
  {}
...
namespace
{
  std::unique_ptr<belnet_context> g_context;

  void
  stream_error(belnet_stream_result* result, int err)
  {
    std::memset(result, 0, sizeof(belnet_stream_result));
    result->error = err;
  }

  void
  stream_okay(belnet_stream_result* result, std::string host, int port, int stream_id)
  {
  ...
  std::optional<belnet_srv_record>
...
{
  std::vector<belnet_srv_record> results;

  int
  LookupSRV(std::string host, std::string service, belnet_context* ctx)
  {
  ...
  void
  IterateAll(std::function<void(belnet_srv_record*)> visit)
  {
...
extern "C"
{
  ...
  char* EXPORT
  belnet_address(struct belnet_context* ctx)
  {
    if (not ctx)
      return nullptr;
    auto lock = ctx->acquire();
  ...
  belnet_context_new()
  {
    return new belnet_context{};
  }

  void EXPORT
  belnet_context_free(struct belnet_context* ctx)
  {
    belnet_context_stop(ctx);
    delete ctx;
  }

  int EXPORT
  belnet_context_start(struct belnet_context* ctx)
  {
    if (not ctx)
      return -1;
    auto lock = ctx->acquire();
    ctx->config->router.m_netId = belnet_get_netid();
  ...
  int EXPORT
  belnet_status(struct belnet_context* ctx)
  {
    if (ctx == nullptr)
      return -3;
    auto lock = ctx->acquire();
  ...
  int EXPORT
  belnet_wait_for_ready(int ms, struct belnet_context* ctx)
  {
    if (ctx == nullptr)
      return -1;
    auto lock = ctx->acquire();
  ...
  void EXPORT
  belnet_context_stop(struct belnet_context* ctx)
  {
    if (not ctx)
      return;
    auto lock = ctx->acquire();
  ...
  int EXPORT
  belnet_inbound_stream(uint16_t port, struct belnet_context* ctx)
  {
    /// FIXME: delete pointer later
    return belnet_inbound_stream_filter(&accept_port, (void*)new std::uintptr_t{port}, ctx);
  }

  int EXPORT
  belnet_inbound_stream_filter(
      belnet_stream_filter acceptFilter, void* user, struct belnet_context* ctx)
  {
  ...
  void EXPORT
  belnet_close_stream(int stream_id, struct belnet_context* ctx)
  {
    if (not ctx)
      return;
    auto lock = ctx->acquire();
      ...
      struct belnet_context* ctx)
  {
    ...
    result->internal = new belnet_srv_lookup_private{};
