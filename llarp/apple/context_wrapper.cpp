...
llarp_apple_init(llarp_apple_config* appleconf)
{
  ...
  try
  {
    ...
    fs::path config_path = config_dir / "belnet.ini";
    ...
    // at this stage of belnet configuration.  So for now we just stick our IPv4 address into it
  ...
  {
    llarp::LogError("Failed to initialize belnet from config: ", e.what());
  }
  return nullptr;
}
...
llarp_apple_start(void* belnet, void* callback_context)
{
  ...
  {
    llarp::LogError("Failed to initialize belnet: ", e.what());
    return -1;
  }
