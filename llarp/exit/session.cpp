...
namespace llarp
{
  namespace exit
  {
    ...
    void
    BaseSession::BlacklistMNode(const RouterID mnode)
    {
      m_MnodeBlacklist.insert(std::move(mnode));
    }
    ...
    MNodeSession::MNodeSession(
        const llarp::RouterID& mnodeRouter,
        std::function<bool(const llarp_buffer_t&)> writepkt,
        AbstractRouter* r,
        size_t numpaths,
        size_t hoplen,
        bool useRouterMNodeKey,
        EndpointBase* parent)
        : BaseSession{mnodeRouter, writepkt, r, numpaths, hoplen, parent}
    {
      if (useRouterMNodeKey)
      {
    ...
    std::string
    MNodeSession::Name() const
    {
      return "MNode::" + m_ExitRouter.ToString();
    }
    ...
    void
    MNodeSession::SendPacketToRemote(const llarp_buffer_t& buf, service::ProtocolType t)
    {
      net::IPPacket pkt;
      if (not pkt.Load(buf))
        return;
